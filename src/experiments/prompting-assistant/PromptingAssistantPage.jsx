...
const PromptingAssistantPage = () => {
  ...
  const [isDragging, setIsDragging] = useState(false); //remove draggedItemId
  ...
  const handleDeleteItem = () => {
    if (itemToDelete.type === "folder") {
      setComponents((prevComponents) => prevComponents.filter((folder) => folder.id !== itemToDelete.id));
      ...
      setComponents((prevComponents) =>
        prevComponents.map((folder) => {
          if (folder.id === itemToDelete.folderId) {
            return {
              ...folder,
              components: folder.components.filter((component) => component.name !== itemToDelete.id),
            };
          }
          return folder;
        })
  ...
  const handleOpenDeleteModal = (type, id, folderId) => {
    ...
    const onDragStart = (start) => {
      setIsDragging(true);
      //remove setDraggedItemId(start.draggableId);
    };

    const onDragEnd = (result) => {
         setIsDragging(false);
        //remove setDraggedItemId(null);
    ...
    const handleInstructionDoubleClick = (component) => {
      setComponents((prevComponents) => {
        return prevComponents.map(folder => {
            const updateComponents = folder.components.map(c => {
                if(c.name === component.name) {
                    return {...c, instruction: component.instruction}
                }
                return c;
            })
            return {...folder, components:updateComponents}
        });
    });
